...
└── src/
    ...
    ├── strings.py                VI/EN translation dictionary for all UI text
    ├── scenarios/
    │   ├── __init__.py
...
Streamlit's script runner replaces the empty-string `sys.path` entry (which resolves to
the working directory `/app`) with the script's own directory (`/app/src`), removing
`/app` from the module search path.  The `ENV PYTHONPATH=/app` line in the Dockerfile
corrects this permanently.  If the error appears after updating code, rebuild and
restart the image:
